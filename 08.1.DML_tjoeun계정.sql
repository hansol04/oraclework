/*
    * DML(DATA MANIPLATION LANGUAGE) : 데이터 조작 언어
      : 테이블에 값을 검색(SELECT), 삽입(INSERT),삭제(DELETE)하는 구문     
*/
-===============================================================================
/*
    * INSERT
...
/*
    2) INSERT INTO 테이블명(컬럼명1,컬럼명2,컬럼명3...) VALUES(값1,값2,값3...)
       : 테이블에 내가 선택한 컬럼에 대한 값만 INSERT 할 때
         그래도 한 행 단위로 추가됨
         -> 선택이 안된 컬럼은 NULL이 들어감. DEFAULT가 지정되어 있으면 NULL이 아닌 DEFAULT값이
    
        * 주의 : NOT NULL 제약조건이 들어있는 컬럼은 반드시 선택하여 값을 넣어야됨
*/
INSERT INTO EMPLOYEE(EMP_ID,EMP_NAME,EMP_NO,HIRE_DATE,JOB_CODE)
...
/*
    3) INSERT ALL
       : 두개 이상의 테이블에 각각 INSERT할 때
         이때 사용되는 서브쿼리가 동일한 경우 -> 한 테이블을 새로운 두 테이블로 나눌 수 있음
*/
...
WHEN SALARY < '3000000' THEN
    INTO EMP_SALARY1 VALUES(EMP_ID,EMP_NAME,SALARY,BONUS)
WHEN SALARY > '3000000' THEN
    INTO EMP_SALARY2 VALUES(EMP_ID,EMP_NAME,SALARY,BONUS)
...
-- EMP_COPY 테이블의 ASIA 지역에서 근무하는 사원들의 보서스를 0.3으로 변경
SELECT EMP_ID
FROM EMPLOYEE_COPY
JOIN DEPARTMENT(DEPT_CODE=DEPT_ID)
JOIN LOCATION ON(LOCATION_ID=LOCATL CODE)
...
COMMIT; -- 다른 테이블을 이용할 때는 COMMIT을 사용해줘야된다?
--==============================================================================
/*
    3. DELETE
       : 테이블의 데이터를 행단위로 삭제
