...
/*
    ...
    AS 서브쿼리;
*/
-- 관리자 계정에서 뷰를 만들 수 있는 권한 주기
GRANT CREATE VIEW TO tjoeun; -- 관리자계정으로 바꿔서 실행해줘야 함
...
*/
-- 전 사원의 사번,사원면,직급명,성별(남/여),근무년수를 조회, SELECT문을 뷰로 정의
...
/*
    * DML 명령어로 조작이 불가능한 경우
    ...
    2) 뷰에 정의되어 있는 컬럼 중에 테이블에 NOT NULL 제약 조건이 지정되어 있는 경우
...
AS SELECT EMP_ID,EMP_NAME,EMP_NO,JOB_CODE,SALARY,SALARY*12연봉
    FROM EMPLOYEE;
...
-- 6) JOIN을 이용하여 여러 테이블을 연결시켜놓는 경우
...
/*
    * VIEW 옵션
    ...
    - FORCE | NOFORCE
      ...
      - WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 값으로만 DML 가능하도록 함
      - WITH READ ONLY : 뷰를 검색만 조회만 가능 (SELECT를 제외한 DML 불가)
    */
