...
>> RESULT SET : SELECT 문을 통해 조회된 결괴물 (조회된 행들의 집합)
...
-- EMPLOYEE테이블에서 사원명, 급여, 보너스, 연봉, 보너스가 포함된 연봉 조회
...
-- EMPLOYEE테이블에서 사원명, 급여, 보너스, 연봉, 보너스가 포함된 연봉 조회
SELECT EMP_NAME,SALARY,BONUS,SALARY*12연봉,(SALARY*BONUS + SALARY)*12
FROM EMPLOYEE;
...
SELECT EMP_ID,EMP_NAME,SALARY,'원'AS단위
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사번,사원명,급여,원,보너스, % 조회
SELECT EMP_ID,EMP_NAME,SALARY,'원'AS단위,BONUS,'%'AS"단위"
FROM EMPLOYEE;
...
-- EMPLOYEE테이블에서 재직중인 사원들의 사번,사원명,이름,입사일 조회
...
-- EMPLOYEE테이블에서 부서코드가 'D6'이거나 급여가 300만원 이상인 사원들의 사원명, 부서코드, 급여를 조회
SELECT EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
...
-- EMPLOYEE테이블에서 사원들 이름에 '하'가 포함되어 있는 사원들의 사원명,주민번호,전화번호 조희
...
-- EMPLOYEE테이블에서 직급코드가 'J7'이거나 'J2' 직원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
-- ORDER BY BONUS NULLS FIRST;   보너스의 오름차순 정렬(NULL값은 맨 앞에 나옴)
