/*
    <트리거 TRIGGER>
    내가 지정한 테이블에 INSERT, UPDATE, DELETE등의 DML문에 의해 변경사항이 생겼을 때
    자동으로 매번 실행할 내용을 미리 정의해 둘 수 있는 객체
    
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터를 삭제한 후 곧바로 탈퇴한 회원만 보관하는 테이블에 INSERT 해야 될 때
    신고횟수가 일정 수를 넘었을 때 해당 회원을 블랙리스트로 처리
    입출고가 발생 했을 때 재고수량을 매번 UPDATE 해야 될 때 
    ...
    * 트리거의 종류
    ...
    - SQL문에 의해 영향을 받는 각 행에 따른 분류
      > STATEMENT TRIGGER(문장 트리거) : 이벤트가 발생하면 딱 한 번만 트리거 실행
      > ROW TRIGGER(행 트리거) : 이벤트가 발생할 때마다 트리거 실행
                            ** FOR EACH ROW 옵션 기술해야됨
                                 -: OLD -> 기존컬럼에 들어있던 데이터
                                 -: NEW -> 새로 들어온 데이터
    * 트리거의 생성 구문
      CREATE [OR REPLACE] TREGGER 트리거명
      BEFORE | AFTER INSERT | UPDATE | DELETE ON 테이브명
      [FOR EACH ROW] 
      [DECLARE
         변수 선언;]
       BEGIN
