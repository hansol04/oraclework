/*
    ...
    - 단일행 함수 : n개의 값을 읽어들여 n개의 결과값 반환(매 행바다 함수 실행)
...
/*
    * LENGTH / LENGTHB => NUMBER로 반환
    ...
    LENGTHB(컬럼|'문자열'):해당 문자열의 byte를 반환
        - 한글 : XE 버전일 때 => 1글자당 3byte (ㄱ,ㅏ,등도 1글자에 해당)
             EE 버전일 때 => 1글자당 2byte 
        - 그위 : 1글자당 1byte
*/
...
SELECT INSTR('JAVASCRIPTJAVAORACLE','A',1,3)FROM DUAL; -- 앞에서부터 3번째 나오는 A의 INDEX 번호
...
/*
    * SUBSTR : 문자열에서 특정 문자열을 추출하여 반환(반환형 : CHARACTER)
      ...
      SUBSTR('문자열',POSITION,[LENGTH])
       ...
       - LENGTH : 추출할 문자의 갯수(생략 시 맨 마지막까지 추출)
*/
...
/*
    * CEIL : 무조건 올림
    ...
    CEIL(NUMBER)
*/
SELECT CEIL(1234.45) FROM DUAL;
SELECT CELL(-123.45) FROM DUAL;
...
-- EMPLOYEE에서 오늘 날짜까지 근무 일수(정수)
SELECT EMP_NAME,HIRE_DATE,CEIL(SYSDATE - HIRE_DATE)근무일수
FROM EMPLOYEE;

-- EMPLOYEE에서 오늘 날짜까지 근무 일수(소수점까지 나옴)
...
-- EMPLOYEE테이블에서 사원명,입사일,정직원이 된 잘짜(입사일에서 6개월 후)조회
SELECT EMP_NAME,HIRE_DATE,ADD_MONTHS(HIRE_DATE,6)
FROM EMPLOYEE;

--------------------------------------------------------------------------------
/*
    * NEXT_DAY(DATE,요일(문자|숫자)):특정 날짜 이후에 가장 가까운 해당 요일의 날짜를 반환해주는 함수
*/
...
--==============================================================================
/*
    * TO_CHAR : 숫자나 날짜를 문자의 타입으로 변환시켜주는 함수
...
/*
    yy는 무조건 앞에 '20'이 붙는다
    rr는 50년을 기준으로 50보다 작으면 앞에 '20'붙이고 50보다 크면 '19'를 붙인다
*/
...
SELECT TO_CHAR(SYSDATE,'MM')
      ,TO_CHAR(SYSDATE,'MON')   -- 영문일 때는 3글자만
      ,TO_CHAR(SYSDATE,'MONTH') -- 영문일 때는 다출력
...
-- SELECT TO_DATE('070204 142530','YYMMDD HH24MISS')FROM DUAL;  -- 변환 후 사용
...
SELECT EMP_NAME,SALARY*(NVL(BONUS,0)+1)*12
FROM EMPLOYEE;
...
-- 사원명, 부서코드(부서에 속해있으면 '부서있음', 부서가 없으면 '부서없음')
SELECT EMP_NAME,NVL2(DEPT_CODE,'부서있음','부서없음')
FROM EMPLOYEE;

-- EMPLOYEE테이블에서 사원명,급여,보너스,성과급(보너스를 받는 사람은 30%를 주고, 보너스를 못받는 사람은 10%)
...
-- 사원명,급여,등급(급여에 따라 등급(5백만원 이상이면 '고급' 5백~3백 '중급' 나머지 '초급'))
...
--전사원의 총 급여의 합
SELECT SUM(SALARY)"총급여의 합"
FROM EMPLOYEE;

--남자사원의 총 급여의 합
SELECT SUM(SALARY)"남자사원의 급여함"
FROM EMPLOYEE
--WHERE SUBSTR(EMP_NO,8,1)='1'OR SUBSTR(EMP_NO,8,1)='3';
WHERE SUBSTR(EMP_NO,8,1)IN('1','3');

-- 부서코드가 D5인 사원이 연봉의 총합
SELECT SUM(SALARY*12)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

--부서코드 D5인 사원의 보너스를 포함한 연봉의 총합
SELECT SUM(SALARY*NVL(1+BONUS,1)*12)"보너스포함 연봉의 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

--전사원의 총 급여의 합 형식 ₩000,000,000
SELECT TO_CHAR(SUM(SALARY),'L999,999,999')"총급여의 합"
FROM EMPLOYEE;
