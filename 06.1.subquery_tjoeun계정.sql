...
/*
    * 서브쿼리의 구분
      ...
      - 단일행 서브쿼리 : 서브쿼리를 실행한 결과 오로지 1개일 때(1행1열)
      - 단중행 서브쿼리 : 서브쿼리를 실행한 결과 여러행 일 때(여러행 1열)
...
/*
    1. 단일행 서브쿼리(SINGLE ROW SUBQUERY)
       - 비교연산자 사용가능
         =, !=, >, < .....
*/
-- 전 직원의 평균급여보다 급여를 더 적게 받는 사원들의 사원명,직급코드,급여조회
...
-- 박정보 사원의 급여보다 더 많이 받는 사원들의 사번,사원명,직급코드,급여조회
SELECT EMP_ID,EMP_NAME,JOB_CODE,SALARY
FROM EMPLOYEE
...
-- 대리 직급임에도 과장의 급여의 최소 급여보다 많이 받는 직원의 사번,사원명,직급,급여 조회
-- 1.과장들의 급여 조회
SELECT SALARY
FROM EMPLOYEE
...
GROUP BY JOB_CODE = 'J5'AND SALARY = 2200000
      OR JOB_CODE = '6' AND SALARY = 2000000
      ...;
...
SELECT EMP_ID,EMP_NAME,SALARY*NVL(1+BONUS,1)*12연봉,DEPT_CODE
FROM EMPLOYEE
WHERE SALARY*NVL(1+BONUS,1)*12>=30000000;

-- WHERE 절에 연봉이라는 별칭을 쓰고 싶으면
SELECT EMP_NAME, 연봉
FROM (SELECT EMP_ID,EMP_NAME,SALARY*NVL(1+BONUS,1)*12연봉,DEPT_CODE
...
-- FROM절의 서브쿼리에 JOB_CODE 컬럼이 없어서 오류남
...
-- 전 직원들중 급여를 가장 많이 받는 사원의 상위 5위까지 가져오기
...
FROM(SELECT DEPT_CODE,CEIL(AVG(SALARY))평균급여
     FROM EMPLOYEE
     GROUP BY DEPT_CODE
     ORDER BY 평균급여 DESC)        --DESC : 내림차순 정렬
WHERE ROWNUM <=3;
...
/*
    *순위 매기는 함수
    ...
    - RANK() OVER(정렬기준) : 동일한 순위 이후의 등수를 동일한 인원수 만큼 거너뛰어 순위 계산
